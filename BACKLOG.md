...
5. ~~**NB-10 widget target**~~ — target WAS wired 2026-07-05 (`68a518585`) but ⚑ **the widget was DROPPED from Build 81 on 2026-07-07** (`7703c1602` — decoupled from the App target, `.appex` out of the archive) → **Build 81 submitted to App Review WIDGET-LESS**. The widget target + Swift are preserved for **Build 82** (de-scope, not deletion); the "archive via Organizer" action is void for 81 (single-target now → `ship:ios` works). Re-wiring the widget for Build 82 is the remaining action.
