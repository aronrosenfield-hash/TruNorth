...
| **NB-2 🔑** | **B-23 — wire `enriched.*` into scoring** | The load-bearing lever. VERIFIED `company.enriched.*` (4,380/12,845 files) is read by **0** scoring paths (server `rebake-scoring.mjs`/`finalize-bundle.mjs` + client `computeScore`/`scoreCat`: 0 `enriched` tokens). Pick which dims score → rebake → `audit-grade-drift.mjs` (rule #16). Folds in B-12 (tax). | ⬜ decision+rebake |
| **NB-3** | **B-63 — strip CC-BY-NC enrichment** | VERIFIED unrun: all 6 NC augment files + 5 writers + the Fashion-Rev sub-score still present. ⚠️ the stale-comment fix (`fec17d0e2`) was LOST in consolidation — lives only on `claude/wizardly-franklin-fb8226`; main still says "currently free … in bounds" (`opensanctions-fetch.mjs:22`, `opensanctions-monthly.yml`). Cherry-pick or redo. | ⬜ next-build |
| **NB-4** | **B-67 — GJF data strip** | Run `vt-strip-gjf.mjs` (on `claude/wizardly-franklin-fb8226`, **already pushed to origin**) AFTER the NB-2 rebake so narratives aren't re-written. | ⬜ after NB-2 |
| **NB-5** | **B-64 tail — fix 3 crons** | VERIFIED root cause: `score-rebake-weekly` + `cruelty-free-merge-weekly` hard-fail <25s — workflows lack an `npm ci` step → `minisearch` import dies (`finalize-bundle.mjs:25`). `epa-echo-weekly` is a separate 1h timeout-hang. Add install step to the two + a budget to ECHO. | ⬜ quick |
| **NB-6** | **E-6 — reconcile source count** | VERIFIED the in-app Sources tab prints **"200+" and "105" in the same paragraph** (`App.jsx:7494/7559`); `docs/SOURCES.md` stale (102 claimed/137 rows/Jun-7); landing mixes 190+/170+. Pipeline = 168 workflows + 225 fetchers → keep **200+** as the one headline, relabel 105 as curated subset, regen SOURCES.md. | ⬜ copy+doc |
...
| **NB-8** | **Triage 8 open PRs** | Close dups #111 (=#114) & #95 (=#97); close #96 opensanctions (conflicts w/ B-63 drop); #115 parent-map guard is CONFLICTING — rebase or close; decide #105 (paywall flip), #72 (health/pharma 633 brands), #114 (la-county data). | ⬜ triage |
| **NB-9** | **(optional) light the Fed-Reserve card** | `fed-reserve-monthly` has NEVER run on main → the Bank-penalties sub-card is dark for all brands. Run fetch+merge once. Pure data, no code change. | ⬜ optional |
...
App is LAUNCHED (Build 75); these are the live calls before the next build — detail in **▶ NEXT BUILD** above:
1. **B-23 scoring wire** (NB-2) — which `enriched.*` dimensions become real grade inputs? The one big lever.
2. **E-1 scoring-flags** (NB-7) — flip the runtime flag on (24h watch) or formally defer.
3. **Open-PR triage** (NB-8) — close dups #111/#95 + opensanctions #96; rebase-or-close #115; decide #105 (paywall) + #72 (health/pharma data).
4. **B-63 / B-67 license strips** (NB-3/NB-4) — confirm the NC drop + GJF strip go in this build.
...
| **B-68** | Land parallel-session branches (PR #115 + GJF) onto main | S · **post-launch** | (1) **PR #115** (brand-parent-map Wikidata collision guard, `claude/determined-taussig-cd52f1`) is **OPEN but CONFLICTING (needs rebase)** — `scripts/lib/parent-map-guards.mjs` drops 12 bad Wikidata edges (asna→GM collisions, BlackRock/Invesco/Pershing shareholder artifacts); map 6,716→6,704, no rebake. Brings **E-11** (already logged done on that branch). **Rebase to resolve the conflicts, then merge.** (2) The GJF branch (B-67) is local-only — push it. (3) ✅ Branch divergence RESOLVED 2026-06-27 — repo consolidated to one clean `main`. |
