...
| **B-30b** | UPS resolves to wrong VT slug `upstream-rehabilitation` | S | Existing v1 mismatch surfaced during B-30 dry-run. Needs slug-aliases.json entry. |
| **B-37b** | Rewrite `atf-fetch.mjs` to new FFL schema (paused-cron prerequisite) | M | New merger expects `public/data/atf-ffl.json`; old fetcher publishes wrong file. Re-enable atf-monthly.yml only after this lands. |
| **B-38** | ~~News-extract pipeline producing 0 high-signal items~~ | ✅ done 2026-06-06 | Shipped (commit 7a15103be). Root cause: Google News substring search returned avalanches of non-trademark hits (lays-bare, Mars-planet, etc.); AI extractor correctly rejected all 200/200 as "not about brand" → items: []. Fix: normalizeForMatch() (apostrophes + accents), NEEDS_CONTEXT_BRANDS w/ business-word requirement, per-brand NEGATIVE_CONTEXT phrases. Replay drops 12% noise; recovers Estée Lauder, McDonald's, etc. |
| **B-36b** | Diagnose loadtest 94% failure rate | S | Run `27063141991` returned http_req_duration p95=42ms (fast) but http_req_failed=94.43%. Suspect: B-35 geo-block middleware affecting GH runner IPs, OR Vercel rate-limit on 256 req/sec sustained, OR k6 check() validating against content that changed. Need to inspect raw.json artifact. |
