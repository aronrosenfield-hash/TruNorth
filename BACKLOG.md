...
| **B-30** | **Expand existing VT (Violation Tracker) coverage** | M | Already integrated via `enriched.laborAPI.violationTracker` (Good Jobs First). 316/11,209 companies covered with rich data: total penalty $, offense groups, primary offenses. Today's research re-confirmed VT is the right path — it aggregates EEOC + OSHA + DOL + state AG settlements per parent company. Worth expanding to: more recent records (last 6 mo cutoff for "active" flag), per-state breakdown, year-over-year trend. |
| **B-37** | **🔴 Rebuild ATF FFL ingestion with entity resolution** | M-L | Surgical scrub on 2026-06-05 removed 57 false-positive gun-manufacturer flags (Uber, AMD, HP, Microsoft, Fox Corp, TaylorMade-golf, etc.) by deny-listing categories where an FFL match is impossible. The systemic fix needs: (1) require CIK/ticker match for public companies, not name fuzzy-match; (2) maintain an allow-list of consumer brands legitimately holding FFLs (L.L.Bean, Orvis, Cabelas, Walmart sporting-goods); (3) optional manual review queue for ambiguous matches before they hit production. Until shipped, the ATF cron should stay paused — currently still runs weekly and would re-pollute the data. Scrub script: `scripts/scrub-bogus-firearms-ffl.mjs` (idempotent — safe to re-run after each cron). |
