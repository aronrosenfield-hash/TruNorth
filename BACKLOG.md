...
| E-6 | **SOURCES_DATA sync** | ~1h | In-app Sources tab lists 100 sources; `docs/SOURCES.md` has ~150; landing claims 200+. User-visible — reconcile before PH eyes on it. ⚠️ Now further out of sync after Build-76 (+7 new pipelines + 3 revived fetchers + the weekly enriched workflow). Reconcile: in-app Sources tab (100) vs `docs/SOURCES.md` (~150) vs landing (200+) — fold in the new enriched sources when this is next touched. |
...
| **B-67** | GJF / Good Jobs First "Violation Tracker" license gate + data strip | M · **post-launch** | ⚠️ Paid-app license exposure: the "Federal penalties" reveal data is **Good Jobs First** (NOT government primaries). **DISPLAY already gated off** 2026-06-27 (`SHOW_FEDERAL_PENALTIES=false`, commit `d19b9dc92`) + scraper quarantined. Data-strip script `scripts/vt-strip-gjf.mjs` **BUILT + sandbox-tested** (`039dbe769`) but **NOT YET RUN**. ⚠️ Both commits live on **local-only** branch `claude/wizardly-franklin-fb8226` (NOT pushed, no PR, 56 behind main) — **push/land before the worktree is pruned or this work is lost.** **Sequencing:** fire the strip ONLY AFTER the Build-76 enriched rebake commits + any B-23 rebake (narratives left untouched → grades don't move). Resume: "run the GJF data strip". |
| **B-68** | Land parallel-session branches (PR #115 + GJF) onto main | S · **post-launch** | (1) **PR #115** (brand-parent-map Wikidata collision guard, `claude/determined-taussig-cd52f1`) is **OPEN + MERGEABLE** — `scripts/lib/parent-map-guards.mjs` drops 12 bad Wikidata edges (asna→GM collisions, BlackRock/Invesco/Pershing shareholder artifacts); map 6,716→6,704, no rebake. Brings **E-11** (already logged done on that branch). **Merge it.** (2) The GJF branch (B-67) is local-only — push it. (3) ⚠️ Reconcile the feat↔main divergence: Build-76 pipeline files are committed on main but exist as untracked `??` dupes on this feat branch — clean up the working tree here rather than re-committing. |
