...
## ▶ NEXT BUILD (Build 81, iOS) — pre-ship worklist (updated 2026-07-05)

> ⚠️ **The iOS binary on main is now v1.1 / Build 81** (`CURRENT_PROJECT_VERSION = 81`, both App + widget targets; repo-bumped `f30acd7be`, **not yet archived/uploaded**). Build 80 was the last one actually shipped via `ship:ios` (2026-07-04). **✅ NB-10 CLOSED — the widget Xcode target is now wired** (`68a518585`, verified on sim) and **Build 81 is the first build that carries the widget** → it must be **archived via Xcode Organizer, NOT `ship:ios`** (script not yet multi-target-aware — see NB-10 / the widget-ship caveat). **The current gate: Aron is device-testing** (diligence mega-batch + retention + charity re-grade + the now-wired widget) — esp. **C5 server-side entitlement** (needs a Sandbox purchase/cancel) — before submitting a v1.1 build for App Review. Everything else below was reconciled 2026-06-27 — re-verify before acting.
...
4. ~~**NB-1 iOS bump**~~ — ✅ moot; binary is now v1.1 / Build **80** on main (Builds 79 + 80 shipped 2026-07-04, next ship = 81). The pending gate is Aron device-testing **Build 80** (esp. **C5 server-side entitlement** — needs a Sandbox purchase/cancel) before submitting a v1.1 build for App Review.
5. ~~**NB-10 widget target**~~ — ✅ **DONE 2026-07-05** (`68a518585`): target wired, App Group set, verified on sim; repo bumped to **Build 81** (`f30acd7be`). Remaining action = Aron **archives Build 81 via Xcode Organizer** (NOT `ship:ios`) to actually carry the widget onto TestFlight.
