...
>
> **Last updated:** 2026-08-01 (daily doc-sync) — **the biggest working day since 07-21 and the end of a 10-day human-commit drought: 11 human commits + 11 bot crons, 7 v1.2 items closed, and the scoring engine moved for the first time since R7.1.** 🟢 **v1.1 Build 81 stays the LIVE App Store build; next iOS ship = Build 82** (no `ios/` or `android/` changes today; Android still scaffold-only). **CATALOG — counted from `index.json`: 12,830 tracked / 3,060 graded** (A 62 · B 1,179 · C 1,053 · D 529 · F 237), 9,770 "?" — **+6 graded**, from two separate grade-moving commits, thresholds untouched.
>
> 🚨 **ACTION FOR YOU — B-115 IS COMMITTED BUT NOT PUSHED.** `a8ba45a7c` exists **only in the local clone**; `origin/main` does not have it and the **live site still scores the old `execPay`**. At sync time the clone was **9 behind / 1 ahead**. This sync deliberately did **not** pull, rebase, or push, because rebasing B-115 over those 9 bot data commits **will conflict** on `index.json` / `meta.json` / `search-index.json` and the overlapping company files — that is a merge you want to drive yourself, not one an unattended job should attempt. **Your push is what takes "Pay & Tax" live.** Suggested: `git pull --rebase origin main`, resolve the bundle files by re-running the rebake, verify the drift, then push.
>
> **✅ CLOSED TODAY (7):** **B-104** commit `audit-ungraded.mjs` as the coverage instrument (`d39dcc8e2`) · **B-105** watchdog rewrite (`e7c90c7fd`) · **B-108** retire the duplicate Canada Competition Bureau pipeline (`fc0b4aa0d`, −636 lines) · **B-113** `very_poor` enum normalization (`539b0947c`) · **B-115** Pay & Tax (`a8ba45a7c`, **unpushed**) · **B-120** cron destagger (`9f4bf87b2`) · **QW-17/QW-18** funnel schema hygiene + `grade` on `scanner_match` (`31c1d3e43`). Plus **`c0a7450f2`** — one canonical brand count (**"3,000+ graded / 12,800+ tracked"**) across App.jsx, MarketingLanding, OnboardingFlow, `index.html`, `llms.txt` and both SEO APIs. **"2,800+" is retired copy; do not reintroduce it.**
>
> **⚠️ B-115 — what actually landed, and a projection to correct.** `execPay` is now **"Pay & Tax"**: `rebake-scoring.mjs` gained `taxAvoidanceScore()` (ITEP federal effective rate, **penalize-only per your option-B call** — median ≈15.8% sits at neutral 50, **capped at 50** so paying tax never lifts a grade; losses → `null`; ≥2 zero-tax years → 8; GAAP/`secTax` deliberately excluded) and `payTaxScore()` (pay + tax averaged when both exist), and `parsePayRatio()` now also reads `enriched.execPay.payRatio` where ~15 brands' real SEC ratios were dark. **Verified from `index.json`, not from the commit message: graded 3,059 → 3,060 (+TD Ameritrade), 15 up / 15 down, 140 brands' `csc.execPay` value changed;** Duke Energy, Southern Co, Con Edison, T-Mobile, Stryker, Lumen all C→D; 28/28 tests, clean `vite build`. **This item was scoped as "up to 927 brands gain an execPay signal" — that was wrong. Actual `csc.execPay` coverage moved 940 → 941.** The win was signal **quality** (140 rescored off SEC/ITEP primaries instead of enum guesses), not coverage. **Quote 140.** Treat the same "up to N" style estimate on **B-116 (393)** and **B-119** with the same skepticism until measured.
>
> **🔧 CRON HEALTH — both silent-death defects are now fixed in code; what's left is verification + 15 individual fetcher breaks.** **B-105** made `cron-health-daily` cadence-agnostic: it now takes the **latest scheduled run of every workflow**, flags `failure`/`cancelled`/`timed_out`/`startup_failure`, rewrites one rolling issue's body, and **never auto-closes on a quiet window**. Its first run opened **issue #155 listing 35 unhealthy crons (20 cancelled + 15 hard failures)** on a day the old watchdog read green. ⚠️ **This retires the old "an empty issue list means nothing" rule — #155's BODY is now the live health list. Read the body, not the count.** **B-120** found the cancellation root cause: **all ~126 data workflows shared one `concurrency: data-pipeline-commit` group** while **~90 were scheduled in the 04:00–07:00 UTC window**, so they cancelled each other as superseded (`trending-refresh` ran **62s against a 30-min cap** and still came back `cancelled` — proof it was never a timeout). Fixed by destaggering + isolating long-runners. ⚠️ **NOT YET VERIFIED: #155 was written 3 minutes AFTER the destagger landed and its rows are historic. Confirm on the next scheduled cycle.** Also ignore #155's `canada-comp-monthly` row — B-108 deleted that workflow 10 minutes after the issue was written.
>
...
> **Measured baseline:** 12,830 tracked · **3,060 graded** · 9,770 "?" (76.1%) *(updated 2026-08-01 after B-113 +5 and B-115 +1; the original audit baseline was 3,054)*. A brand grades iff ≥1 of
...
- **B-107 — triage the 15 hard-FAILURE crons** (distinct from the 20 cancellations B-120 fixed). *(WS-B, L —
  ⭐ **now the highest-value open ops item**, and with B-105 shipped it finally has a trustworthy live list:
  **read the body of issue #155**, which the watchdog rewrites every day. Two rows there are already stale by
  construction: `canada-comp-monthly` was deleted by B-108, and every `cancelled` row predates the B-120
  destagger — **re-read #155 after one full scheduled cycle before starting triage**, since the real
  hard-failure list may be shorter than 15.)*
...
- **B-115 ✅ DONE 2026-08-01 (`a8ba45a7c`) — ⚠️ COMMITTED BUT NOT PUSHED; the live site still scores the old
  execPay.** *(WS-A, M, grades, decision RESOLVED — Aron chose option B, penalize-only)* `execPay` is now
  **"Pay & Tax"**: `taxAvoidanceScore()` reads the **ITEP** federal effective rate (`0%→8 · 5%→20 · 10%→36 ·
  ≥15%→50`, **capped at 50** so paying tax never lifts a grade; losses → `null`; ≥2 zero-tax years → 8;
  GAAP/`secTax` deliberately excluded as noisy), `payTaxScore()` averages pay + tax when both exist, and
  `parsePayRatio()` now also reads `enriched.execPay.payRatio` where ~15 brands' real SEC ratios were dark.
  Runs before the neutral guard so a tax-only signal still scores; App.jsx reads the baked `csc.execPay`, so
  both scorers stay in sync with no runtime edit. **Measured drift (from `index.json`): graded 3,059 → 3,060
  (+TD Ameritrade), 15 up / 15 down, 140 brands' `csc.execPay` changed;** Duke/Southern/ConEd/T-Mobile/
  Stryker/Lumen all C→D; 28/28 tests, clean build. ⚠️ **The "up to 927 brands" estimate was wrong —
  `csc.execPay` coverage moved 940 → 941.** The win was signal quality (140 rescored off primaries), not
  coverage. **Quote 140.** 🚨 **Remaining action is Aron's: rebase over the 9 newer bot commits (expect
  conflicts on `index.json`/`meta.json`/`search-index.json`) and push. That push is the deploy.**
