...
> ⚠️ **This table is a SCHEDULE, not a health report** (flagged 2026-07-27). Five of the pipelines listed below have not delivered in over a month: **GDELT** (below) and **FRA** have produced **no output file at all — ever** (`public/data/gdelt.json` and `public/data/fra-incidents.json` do not exist on disk); **FAA** last refreshed 2026-07-20; **FSIS** + **FSIS-DW** have 403'd weekly since June. See **B-97 / B-98 / B-99**. Verify against `gh run list` and the output file's mtime before citing any row here as live.
