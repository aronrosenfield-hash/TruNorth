...
>
> **CATALOG — unchanged for a FIFTH day; still two numbers, both counted from `index.json`.** **LOCAL (has B-115): 12,830 tracked / 3,065 graded**, 9,765 "?". **`origin/main` (no B-115, what the live site serves): 12,830 tracked / 3,059 graded**, 9,771 "?". **Re-counted today at `118700e87` — today's 3 data commits moved ZERO grades** (none of them touched `index.json` at all). The 6-brand gap IS B-115 and nothing else. ⚠️ **The 08-01 header's "3,060 / A 62" was measured at `a8ba45a7c` — the REJECTED first pass — so it is not a valid baseline for anything. The true pre-B-115 baseline is `c0a7450f2` = 3,059 / A 64.**
>
...
>
> 🚨 **ACTION FOR YOU — B-115 IS COMMITTED BUT NOT PUSHED (day 5).** ⏳ **TODAY'S NUMBER: `51 behind / 9 ahead` on a fresh 08-06 fetch. The trend: 24 behind (08-02) → 34 → 42 → 48 → 51 today.** ✅ **Good news worth acting on: today added only 3 commits to the debt, the smallest daily increment since the count started (prior days ran 6–10). The rebase is growing more slowly than it was — that makes today a cheaper day to do it than yesterday, not a reason to keep waiting.** The rest of this item is unchanged from 08-05 and still accurate:
>
...
>
> 🔴 **STILL OPEN — ALL OUTBOUND EMAIL IS DOWN (B-121, day 5, and you now have 3 DAYS).** ⏰ **Re-verified 2026-08-06: `weekly-digest`'s last run is still the 2026-08-02T18:58Z failure — it has not run since, and nothing has changed. Exactly ONE send has been missed. The next send is Sunday 2026-08-09.** **This is the single most deadline-bound item on the board: it is the only open item where doing nothing produces a new, visible, external failure on a known date.** Decide the path (upgrade MailerLite, or move the send to Resend — already wired for `trunorthapp.com`) by 08-08 so there is a day to test. The rest of this item is unchanged from 08-02 and still accurate:
>
...
>
> 🔴 **STILL OPEN — `bis-entity-list-weekly` is dead on an EXPIRED TLS CERTIFICATE at the source (B-122, day 9).** ⏳ **RE-PROBED 2026-08-06 from Aron's Mac — NO CHANGE from 08-05, and that is the useful result.** `api.trade.gov` (the host our fetcher actually calls) still returns `certificate has expired`, `ssl_verify_result=10`. `data.trade.gov` still presents a **valid** cert (`ssl_verify_result=0`, answers `301`). **Two days of identical readings confirm the host-by-host reissue has stalled on our host specifically — this is not a rollout still in motion.** 🔬 **The probe rule stands and is cheap (10 seconds): probe the exact HOST, never the domain. "trade.gov is fixed" is not a checkable claim.** **Nothing changes the plan: expect the ~08-10 run to fail, and decide on the DATE, not the failure.** The rest of this item is unchanged from 08-03/08-05 and still accurate:
>
...
- **B-123 🔴 NEW 2026-08-05 — 4 crons `git add` a path that `.gitignore` ignores, so they fetch real data and
  ...
  **What happens:** commit **`f356963c2` (2026-06-11, "fix(infra): C1+C2 — untrack 398MB pipeline `_cache`
  ...
  `ca100-annual.yml:59`. `git add` on an ignored path prints *"The following paths are ignored by one of your
