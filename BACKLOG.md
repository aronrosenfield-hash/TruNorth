...
## ▶ NEXT BUILD (Build 81, iOS) — pre-ship worklist (updated 2026-07-04)

> ⚠️ **The iOS binary is now v1.1 / Build 80** (Builds 79 + 80 both shipped 2026-07-04; next = **81**). NB-1 below (the 75→76 bump) is long **DONE/moot**. **The current gate: Aron is device-testing Build 80** (diligence mega-batch + retention + charity re-grade + widget scaffold) — esp. **C5 server-side entitlement** (needs a Sandbox purchase/cancel) — before submitting a v1.1 build for App Review. ⚠️ **New pre-ship item: the widget Xcode target isn't created yet** (`docs/widget-setup.md`) — Build 80 ships with NO widget; `ios/App/TruNorthWidget/*` are inert until the target is wired in Xcode (see NB-10). Everything else below was reconciled 2026-06-27 — re-verify before acting.

| # | Item | What / why (verified) | Status |
|---|---|---|---|
| **NB-10 🆕** | **Wire the widget Xcode target** | Widget code-complete (`7f3886bce`: `TruNorthWidget.swift`, entitlements, `src/lib/widget.js`, `@capacitor/preferences` bridge) but the **Widget Extension target isn't created in Xcode** → Builds 79/80 ship without it. Follow `docs/widget-setup.md` to add the target, then a build actually carries the widget. | ⬜ manual Xcode step (Aron) |
...
5. **NB-10 widget target (🆕)** — the Home/Lock-Screen widget is code-complete (`7f3886bce`) but the Widget Extension **Xcode target isn't created yet** (`docs/widget-setup.md`), so Build 80 ships without it. Wire the target (manual Xcode step) before a build can carry the widget.
