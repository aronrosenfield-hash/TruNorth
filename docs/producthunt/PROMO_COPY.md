# Coming Soon promotion — paste-ready copy

After your Coming Soon page is live, share this URL aggressively for 1-2 weeks before launch. Goal: 100+ subscribers = ~30+ guaranteed upvotes hour 1.

Replace `[YOUR-PH-URL]` with the actual Coming Soon URL ProductHunt gives you.

---

## Twitter / X — pinned tweet

```
TruNorth is launching on @ProductHunt soon.

Grades 11,000+ companies on politics, environment, labor, animal testing & more — using only public records (FEC, EPA, OSHA).

No vibes. No spin. Just journalism.

Subscribe for the launch ↓
[YOUR-PH-URL]
```

## LinkedIn — pinned post

```
After a year of building, TruNorth is launching on Product Hunt soon.

The problem: every "ethical shopping" app I tried felt like vibes — opinions dressed up as ratings.

So I built TruNorth differently. It pulls from primary sources only:
• FEC for political donations
• EPA for environmental enforcement
• OSHA for labor violations
• NLRB for union cases
• OpenFDA for product recalls
• SEC for exec pay
• 20+ more

11,000 companies graded. Every score traceable to a public-record citation.

You can scan a barcode in-store and get the verdict before you pay.

Free forever. iOS first.

Subscribe on Product Hunt to get notified when we launch ↓
[YOUR-PH-URL]

#startup #consumertech #climate #ethicalshopping #indieMaker
```

## Email signature

Add this line under your current email signature:

```
🚀 Subscribe to our Product Hunt launch: [YOUR-PH-URL]
```

## Personal email blast to your network

```
Subject: I've been building this in my garage and would love your support

Hi [name],

I've been quietly building TruNorth — a consumer values shopping app that grades 11,000+ companies on what they actually do (not what their marketing says).

It pulls from public records only — FEC donations, EPA violations, OSHA citations. Every grade traceable. Free forever. Currently in TestFlight beta.

We're launching on Product Hunt in the next ~2 weeks. Would mean a lot if you'd subscribe to get the launch notification (one email, no spam):

[YOUR-PH-URL]

And if you want early TestFlight access in the meantime, just reply.

Thanks,
Aron
Aron@trunorthapp.com
```

Send to:
- Family + close friends (5-10)
- Former colleagues you still talk to
- Anyone in your network who'd vote on launch day

## Indie Hackers / Hacker News teaser (post 5 days before launch)

In r/SideProject, IndieHackers feed, etc.:

```
Title: Built TruNorth — public-records-only consumer values shopping app — launching on PH soon

Body:
Hey HN/IH,

After a year solo, my consumer values app launches on Product Hunt in about a week. The unique angle: every grade across 11,000 companies traces back to a public record (FEC, EPA, OSHA, SEC, NLRB, OpenFDA). No AI scoring, no scraped opinions.

Subscribe here if interested: [YOUR-PH-URL]

Happy to answer questions about the data pipeline, scoring methodology, or the indie founder journey.
```

---

## Frequency suggestion (don't be spammy)

- **Twitter:** post the pinned tweet, then 1 fresh tweet every 2-3 days with a different angle (data finding, demo GIF, founder story, behind-the-scenes)
- **LinkedIn:** the pinned post + maybe 1-2 follow-ups during the 2 weeks
- **Personal email:** ONCE per recipient. Don't ping the same person twice.
- **Communities:** post in each ONE TIME. Re-posting = ban.
