...
- **B68 — clash-led basket articulation** (2026-06-12, Aron picked options A+C): the headline basket statistic is now the CLASH COUNT, never the aligned-%. Rationale: "aligned" counted only A/B, so strong-stance users (half the population, both directions) opened the app to "0% aligned" when the truth was "mostly neutral, one fixable problem." Today's compass card → serif clash sentence + named clash + brass one-switch projection; Ledger dial → "N clashes · graded / aligned / neutral"; Reveal → "One of your brands clashes with your compass." Scoring untouched and symmetric — presentation only. basketVerdict() in App.jsx is the single source.
