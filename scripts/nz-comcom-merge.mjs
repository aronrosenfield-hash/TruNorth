#!/usr/bin/env node
/**
 * NZ Commerce Commission merge.
 *
 * data/raw/nz-comcom/<date>.json -> data/derived/nz-comcom-augment.json
 */
...
const DERIVED = path.join(ROOT, "data/derived/nz-comcom-augment.json");
...
export function buildAugment(records, knownSlugs = new Set(), maps = { aliases: {}, parents: {} }) {
  ...
  for (const r of records) {
    ...
    const agg = by[slug];
    agg.total_penalty_nzd += r.penalty_nzd || 0;
    agg.action_count      += 1;
...
async function main() {
  ...
  if (!inPath || !existsSync(inPath)) { console.error("Run nz-comcom-fetch.mjs first."); process.exit(2); }
...
if (isMain) {
  main().catch(err => { console.error("nz-comcom-merge failed:", err); process.exit(1); });
}
