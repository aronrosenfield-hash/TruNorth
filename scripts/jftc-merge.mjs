 ...
 * JFTC merge.
 *
 * data/raw/jftc/<date>.json -> data/derived/jftc-augment.json
 */
...
const DERIVED = path.join(ROOT, "data/derived/jftc-augment.json");
...
export function buildAugment(records, knownSlugs = new Set(), maps = { aliases: {}, parents: {} }) {
  ...
  for (const r of records) {
    ...
    const agg = by[slug];
    agg.total_penalty_jpy += r.penalty_jpy || 0;
    agg.action_count      += 1;
...
async function main() {
  ...
  if (!inPath || !existsSync(inPath)) { console.error("Run jftc-fetch.mjs first."); process.exit(2); }
...
if (isMain) {
  main().catch(err => { console.error("jftc-merge failed:", err); process.exit(1); });
}
