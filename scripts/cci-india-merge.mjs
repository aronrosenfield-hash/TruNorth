 ...
 * CCI India merge.
 *
 * data/raw/cci-india/<date>.json -> data/derived/cci-india-augment.json
 */
...
const DERIVED = path.join(ROOT, "data/derived/cci-india-augment.json");
...
export function buildAugment(records, knownSlugs = new Set(), maps = { aliases: {}, parents: {} }) {
  ...
  for (const r of records) {
    ...
    const agg = by[slug];
    agg.total_penalty_inr += r.penalty_inr || 0;
    agg.action_count      += 1;
...
async function main() {
  ...
  if (!inPath || !existsSync(inPath)) { console.error("Run cci-india-fetch.mjs first."); process.exit(2); }
...
if (isMain) {
  main().catch(err => { console.error("cci-india-merge failed:", err); process.exit(1); });
}
