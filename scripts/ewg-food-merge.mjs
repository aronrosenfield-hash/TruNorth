#!/usr/bin/env node
/**
 * ewg-food-merge — roll EWG Food Scores per-product → per-brand summary.
 *
 * Same rollup math + severity heuristic as ewg-skin-deep-merge.mjs,
 * different output file:
 *
 *   data/derived/ewg-food-augment.json
 *
 * Per-slug shape mirrors the skin-deep augment.
 */
...
const OUT_FILE = path.join(ROOT, "data/derived/ewg-food-augment.json");
...
export function rollUpByBrand(productsBySlug, sourceUrl) {
  ...
  for (const [slug, products] of productsBySlug) {
    ...
    out[slug] = {
      slug,
      food_avg_score: Number(avg.toFixed(2)),
      food_worst_score: worst,
      food_pct_flagged: Number(pct_flagged.toFixed(3)),
      food_product_count: sample,
      food_flagged_count: flagged,
...
if (isMain) {
  main().catch(err => {
    console.error("ewg-food-merge failed:", err);
    process.exit(1);
  });
}
