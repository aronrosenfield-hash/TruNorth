#!/usr/bin/env node
/**
 * Step 2 — Merge uk-msa.json into per-company JSON.
 *
 * Reads /public/data/uk-msa.json (produced annually by uk-msa-fetch.mjs)
 * and writes a structured `ukMsa` field into each matching company file
 * under enriched.ukMsa. Honors slug-aliases.json + brand-parent-map.json.
 *
 * Target schema (only set when status === "ok"):
 *   enriched.ukMsa: {
 *     hasStatement:        bool,
 *     statementCount:      int,
 *     latestStatementYear: int|null,
 *     statementUrl:        str,
 *     searchUrl:           str,
 *     lastUpdated:         ISO string,
 *     source:              "uk-msa-registry"
 *   }
 *
 * Brands with no UK MSA statement match are skipped (not merged).
 *
 * Locally: node scripts/uk-msa-merge.mjs
 */
...
const SRC_FILE = path.join(ROOT, "public/data/uk-msa.json");
const COMP_DIR = path.join(ROOT, "public/data/companies");
const META_DIR = path.join(ROOT, "public/data/_meta");
const LOG_FILE = path.join(ROOT, "public/data/_meta/uk-msa-merge-log.json");
...
async function mergeOne(brandEntry, maps, now) {
  ...
  company.enriched.ukMsa = {
    hasStatement:        !!brandEntry.has_uk_msa_statement,
    statementCount:      brandEntry.statement_count ?? 0,
    latestStatementYear: brandEntry.latest_statement_year ?? null,
    statementUrl:        brandEntry.statement_url || null,
    searchUrl:           brandEntry.search_url || null,
    lastUpdated:         now,
    source:              "uk-msa-registry",
  };
  ...
  company.dataLastUpdated.ukMsa = now;
  ...
  return {
    ...
    routed_via,
    status:          "merged",
    statement_count: brandEntry.statement_count,
    latest_year:     brandEntry.latest_statement_year,
  };
}

async function main() {
  const now = new Date().toISOString();
  console.log("UK MSA merge starting...");
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:     now,
    source_file:   "public/data/uk-msa.json",
  ...
  console.log(`   Skipped (no statement): ${skipped.length}`);
  console.log(`   Orphan slugs:           ${orphans.length}`);
  console.log(`   Parse errors:           ${errors.length}`);
}

main().catch(err => {
  console.error("uk-msa-merge failed:", err);
  process.exit(1);
});
