#!/usr/bin/env node
/**
 * Step 2 -- Merge msc.json into per-company JSON.
 *
 * Reads /public/data/msc.json (produced annually by msc-fetch.mjs) and
 * writes the structured `msc` field into each matching company file
 * under enriched.msc. Honors slug-aliases.json + brand-parent-map.json.
 *
 * Target schema (only set when status === "ok"):
 *   enriched.msc: {
 *     isMscCertified: boolean,
 *     mscCategories:  string[],
 ...
 *   }
 *
 * Brands with no MSC match are skipped (the majority).
 *
 * Locally: node scripts/msc-merge.mjs
 */
...
const MSC_FILE = path.join(ROOT, "public/data/msc.json");
const COMP_DIR = path.join(ROOT, "public/data/companies");
const META_DIR = path.join(ROOT, "public/data/_meta");
const LOG_FILE = path.join(ROOT, "public/data/_meta/msc-merge-log.json");
...
async function mergeOne(brandEntry, maps, now) {
  ...
  company.enriched.msc = {
    isMscCertified: brandEntry.is_msc_certified === true,
    mscCategories:  brandEntry.msc_categories || [],
  ...
  company.dataLastUpdated.msc = now;
  ...
  return {
    ...
    routed_via,
    status:       "merged",
    categories:   brandEntry.msc_categories,
...
async function main() {
  ...
  console.log("MSC merge starting...");

  const msc = JSON.parse(await fs.readFile(MSC_FILE, "utf-8"));
  const entries = msc.certifications || [];
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:        now,
    source_file:      "public/data/msc.json",
...
main().catch(err => {
  console.error("msc-merge failed:", err);
  process.exit(1);
});
