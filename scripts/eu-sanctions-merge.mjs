#!/usr/bin/env node
/**
 * Step 2 -- Merge eu-sanctions.json into per-company JSON.
 *
 * Reads /public/data/eu-sanctions.json (produced monthly by
 * eu-sanctions-fetch.mjs) and writes the structured `euSanctions` field
 * into each matching company file. Honors slug-aliases.json +
 ...
 *   }
 *
 * Brands with no EU match are skipped (the vast majority -- usually 100%
 * for a US consumer brand list).
 *
 * Locally: node scripts/eu-sanctions-merge.mjs
 */
...
const EU_FILE   = path.join(ROOT, "public/data/eu-sanctions.json");
const COMP_DIR  = path.join(ROOT, "public/data/companies");
const META_DIR  = path.join(ROOT, "public/data/_meta");
const LOG_FILE  = path.join(ROOT, "public/data/_meta/eu-sanctions-merge-log.json");
...
async function mergeOne(brandEntry, maps, now) {
  ...
  company.euSanctions = {
    isSanctioned:        brandEntry.is_eu_sanctioned === true,
    sanctionedCount:     brandEntry.eu_sanction_count,
    sanctionedPrograms:  brandEntry.eu_sanction_programs,
  ...
  company.dataLastUpdated.euSanctions = now;
  ...
  return {
    ...
    isSanctioned:     brandEntry.is_eu_sanctioned === true,
    sanctionedCount:  brandEntry.eu_sanction_count,
    programs:         brandEntry.eu_sanction_programs,
  };
}

async function main() {
  const now = new Date().toISOString();
  console.log("EU sanctions merge starting...");

  const eu = JSON.parse(await fs.readFile(EU_FILE, "utf-8"));
  const entries = eu.sanctions || [];
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:         now,
    source_file:       "public/data/eu-sanctions.json",
...
main().catch(err => {
  console.error("eu-sanctions-merge failed:", err);
  process.exit(1);
});
