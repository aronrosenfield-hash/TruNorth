 ...
 * UK FCA merge.
 *
 * data/raw/uk-fca/<date>.json -> data/derived/uk-fca-augment.json
 */
...
const DERIVED = path.join(ROOT, "data/derived/uk-fca-augment.json");
...
export function buildAugment(records, knownSlugs = new Set(), maps = { aliases: {}, parents: {} }) {
  ...
  for (const r of records) {
    ...
    const agg = by[slug];
    agg.total_fines_gbp += r.penalty_gbp || 0;
    agg.action_count    += 1;
...
async function main() {
  ...
  if (!inPath || !existsSync(inPath)) { console.error("Run uk-fca-fetch.mjs first."); process.exit(2); }
...
if (isMain) {
  main().catch(err => { console.error("uk-fca-merge failed:", err); process.exit(1); });
}
