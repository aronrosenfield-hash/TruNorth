...
const MBFC_FILE = path.join(ROOT, "data/derived/mbfc-outlet-bias.json");

// Lazy-loaded MBFC outlet bias table (generated by scripts/mbfc-fetch.mjs).
// ~8,700 outlets covering nearly every domain Google News RSS surfaces.
// Hand-curated OUTLET_BIAS below always wins on conflict — MBFC is a
// fallback for the long tail. Schema:
//   { bias, factuality, weight, credibility, fact_driver }
let MBFC_OUTLETS = null;
async function loadMbfc() {
  if (MBFC_OUTLETS !== null) return MBFC_OUTLETS;
  try {
    const raw = await fs.readFile(MBFC_FILE, "utf-8");
    const json = JSON.parse(raw);
    MBFC_OUTLETS = json.outlets || {};
    console.log(`📊 Loaded ${Object.keys(MBFC_OUTLETS).length} MBFC outlets`);
  } catch (err) {
    console.warn(`⚠️  MBFC outlet table not found at ${MBFC_FILE} — falling back to 33-outlet hand map.`);
    MBFC_OUTLETS = {};
  }
  return MBFC_OUTLETS;
}

function resolveOutletMeta(domain) {
  if (!domain) return null;
  // Hand-curated map wins.
  if (OUTLET_BIAS[domain]) return OUTLET_BIAS[domain];
  // Fall through to MBFC's 8,700-outlet table.
  const mbfc = MBFC_OUTLETS && MBFC_OUTLETS[domain];
  if (mbfc) {
    return {
      bias:        mbfc.bias,
      weight:      mbfc.weight,
      fact_driver: mbfc.fact_driver,
      factuality:  mbfc.factuality,
...
async function fetchBrandNews(brand) {
  ...
  try {
    ...
    return validItems.map(it => {
      ...
      const meta = resolveOutletMeta(domain) || { bias: "unknown", weight: 0.3, fact_driver: false };
      return {
