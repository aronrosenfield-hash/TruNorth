 ...
 * UK CMA merge.
 *
 * data/raw/uk-cma/<date>.json -> data/derived/uk-cma-augment.json
 */
...
const DERIVED = path.join(ROOT, "data/derived/uk-cma-augment.json");
...
export function buildAugment(records, knownSlugs = new Set(), maps = { aliases: {}, parents: {} }) {
  ...
  for (const r of records) {
    ...
    const agg = by[slug];
    agg.total_penalty_gbp += r.penalty_gbp || 0;
    agg.action_count      += 1;
...
async function main() {
  ...
  if (!inPath || !existsSync(inPath)) { console.error("Run uk-cma-fetch.mjs first."); process.exit(2); }
  const raw = JSON.parse(await fs.readFile(inPath, "utf-8"));
  const knownSlugs = await loadKnownSlugs(COMPANIES_DIR);
...
if (isMain) {
  main().catch(err => { console.error("uk-cma-merge failed:", err); process.exit(1); });
}
