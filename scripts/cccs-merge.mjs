 ...
 * CCCS merge.
 *
 * data/raw/cccs/<date>.json -> data/derived/cccs-augment.json
 */
...
const DERIVED = path.join(ROOT, "data/derived/cccs-augment.json");
...
export function buildAugment(records, knownSlugs = new Set(), maps = { aliases: {}, parents: {} }) {
  ...
  for (const r of records) {
    ...
    const agg = by[slug];
    agg.total_penalty_sgd += r.penalty_sgd || 0;
    agg.action_count      += 1;
...
async function main() {
  ...
  if (!inPath || !existsSync(inPath)) { console.error("Run cccs-fetch.mjs first."); process.exit(2); }
...
if (isMain) {
  main().catch(err => { console.error("cccs-merge failed:", err); process.exit(1); });
}
