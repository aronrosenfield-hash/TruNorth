#!/usr/bin/env node
/**
 * CCCS (Competition and Consumer Commission of Singapore) — antitrust,
 * cartel, and merger enforcement under the Competition Act.
 *
 * Source: https://www.cccs.gov.sg/cases-and-commitments
 *
 * CCCS publishes decisions and media releases but no structured feed.
 * This fetcher reads:
 *   - JSON from --url, or
 *   - the bundled CSV fixture under scripts/fixtures/cccs/.
 *
 * Fields per row:
 *   respondent   string
 *   action_type  string   (Cartel | Merger | Abuse of dominance | ...)
 *   penalty_sgd  number   (financial penalty in SGD)
 *   date         ISO date
 *   url          string
 *
 * Raw output: data/raw/cccs/<YYYY-MM-DD>.json
 */
...
const RAW_DIR = path.join(ROOT, "data/raw/cccs");
const FIXTURE = path.join(ROOT, "scripts/fixtures/cccs/sample.csv");

const SOURCE_URL = "https://www.cccs.gov.sg/cases-and-commitments";
const UA = "TruNorth-CCCS/1.0 (+https://www.trunorthapp.com)";
...
export function rowsToRecords(rows) {
  ...
  const idx = {
    ...
    penalty_sgd: header.indexOf("penalty_sgd"),
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    out.push({
      ...
      penalty_sgd: parseSGD(r[idx.penalty_sgd]),
...
export function jsonToRecords(items) {
  ...
  for (const it of items ?? []) {
    ...
    out.push({
      ...
      penalty_sgd: parseSGD(it.penalty_sgd ?? it.fine ?? it.penalty ?? 0),
...
async function loadInput() {
  if (APPLY && URL_IN) {
    ...
    if (!res.ok) throw new Error(`CCCS fetch ${res.status} ${res.statusText}`);
...
async function main() {
  console.log(`CCCS fetcher (${APPLY ? "APPLY" : "DRY"})`);
...
if (isMain) {
  main().catch(err => { console.error("cccs-fetch failed:", err); process.exit(1); });
}
