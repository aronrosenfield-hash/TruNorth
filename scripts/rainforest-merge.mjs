#!/usr/bin/env node
/**
 * Step 2 -- Merge rainforest.json into per-company JSON.
 *
 * Reads /public/data/rainforest.json (produced annually by
 * rainforest-fetch.mjs) and writes the structured `rainforestAlliance`
 * field into each matching company file under enriched.rainforestAlliance.
 * Honors slug-aliases.json + brand-parent-map.json.
 *
 * Target schema (only set when status === "ok"):
 *   enriched.rainforestAlliance: {
 *     isRainforestAllianceCertified: boolean,
 *     raCategories:                  string[],
 ...
 *   }
 *
 * Brands with no RA match are skipped (the majority).
 *
 * Locally: node scripts/rainforest-merge.mjs
 */
...
const RA_FILE  = path.join(ROOT, "public/data/rainforest.json");
const COMP_DIR = path.join(ROOT, "public/data/companies");
const META_DIR = path.join(ROOT, "public/data/_meta");
const LOG_FILE = path.join(ROOT, "public/data/_meta/rainforest-merge-log.json");
...
async function mergeOne(brandEntry, maps, now) {
  ...
  company.enriched.rainforestAlliance = {
    isRainforestAllianceCertified: brandEntry.is_rainforest_alliance_certified === true,
    raCategories:                  brandEntry.ra_categories || [],
  ...
  company.dataLastUpdated.rainforestAlliance = now;
  ...
  return {
    ...
    routed_via,
    status:       "merged",
    categories:   brandEntry.ra_categories,
...
async function main() {
  ...
  console.log("Rainforest Alliance merge starting...");

  const ra = JSON.parse(await fs.readFile(RA_FILE, "utf-8"));
  const entries = ra.certifications || [];
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:        now,
    source_file:      "public/data/rainforest.json",
...
main().catch(err => {
  console.error("rainforest-merge failed:", err);
  process.exit(1);
});
