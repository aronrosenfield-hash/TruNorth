#!/usr/bin/env node
/**
 * Canada Competition Bureau merge — writes the `canadaCompBureau` field
 * into each matching company JSON under `enriched`.
 *
 * Reads /public/data/canada-comp-actions.json (produced monthly by
 * canada-comp-fetch.mjs) and writes the structured field per brand.
 *
 * Target schema:
 *   enriched.canadaCompBureau: {
 *     totalActions5y:        number,
 *     totalFinesCad:         number,
 *     sampleActions:         [{ title, url, date, snippet, fine_cad, action_types }],
 *     lastUpdated:           ISO,
 *     source:                "canada-competition-bureau",
 *     sourceUrl:             string,
 *   }
 *
 * Honors slug-aliases + brand-parent-map (same pattern as fed-reserve-merge,
 * occ-merge, msha-merge). Skips entries with no actions — vast majority of
 * non-Canadian-active brands return zero results.
 *
 * Locally: node scripts/canada-comp-merge.mjs
 */
...
const SRC_FILE   = path.join(ROOT, "public/data/canada-comp-actions.json");
const COMP_DIR   = path.join(ROOT, "public/data/companies");
const META_DIR   = path.join(ROOT, "public/data/_meta");
const LOG_FILE   = path.join(ROOT, "public/data/_meta/canada-comp-merge-log.json");
...
async function mergeOne(entry, maps, now) {
  if (entry.status !== "ok" || !entry.total_canada_actions_5y) {
  ...
  company.enriched.canadaCompBureau = {
    totalActions5y: entry.total_canada_actions_5y,
    totalFinesCad:  entry.total_fines_cad || 0,
  ...
  company.dataLastUpdated.canadaCompBureau = now;
  ...
  return {
    ...
    routed_via,
    status:   "merged",
    actions5y: entry.total_canada_actions_5y,
    fines_cad: entry.total_fines_cad || 0,
  };
}

async function main() {
  const now = new Date().toISOString();
  console.log("Canada Competition Bureau merge starting...");
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:        now,
    source_file:      "public/data/canada-comp-actions.json",
    releases_scanned: src.releases_scanned,
    ...
    top_by_actions:   merged
      .slice()
      .sort((a, b) => b.actions5y - a.actions5y)
      .slice(0, 20)
      .map((m) => ({
        brand:     m.brand,
        target:    m.target,
        actions5y: m.actions5y,
        fines_cad: m.fines_cad,
      })),
  }, null, 2));
...
main().catch((err) => {
  console.error("canada-comp-merge failed:", err);
  process.exit(1);
});
