#!/usr/bin/env node
/**
 * Step 2 -- Merge climate-neutral.json into per-company JSON.
 *
 * Reads /public/data/climate-neutral.json (produced annually by
 * climate-neutral-fetch.mjs) and writes the structured `climateNeutral`
 * field into each matching company file under enriched.climateNeutral.
 * Honors slug-aliases.json + brand-parent-map.json.
 *
 * Target schema (only set when status === "ok"):
 *   enriched.climateNeutral: {
 *     isClimateNeutralCertified: boolean,
 *     sinceYear:                 number | null,
 *     offsetTonsYear:            number | null,
 *     sourceUrl:                 string,
 *     lastUpdated:               ISO string,
 *     source:                    "climate-neutral"
 *   }
 *
 * Brands with no Climate Neutral match are skipped (the majority).
 *
 * Locally: node scripts/climate-neutral-merge.mjs
 */
...
const CN_FILE  = path.join(ROOT, "public/data/climate-neutral.json");
const COMP_DIR = path.join(ROOT, "public/data/companies");
const META_DIR = path.join(ROOT, "public/data/_meta");
const LOG_FILE = path.join(ROOT, "public/data/_meta/climate-neutral-merge-log.json");
...
async function mergeOne(brandEntry, maps, now) {
  ...
  company.enriched.climateNeutral = {
    isClimateNeutralCertified: brandEntry.is_climate_neutral_certified === true,
    sinceYear:                 brandEntry.since_year ?? null,
    offsetTonsYear:            brandEntry.offset_tons_year ?? null,
  ...
  company.dataLastUpdated.climateNeutral = now;
  ...
  return {
    ...
    offset_tons_year: brandEntry.offset_tons_year ?? null,
  };
}

async function main() {
  const now = new Date().toISOString();
  console.log("Climate Neutral merge starting...");

  const cn = JSON.parse(await fs.readFile(CN_FILE, "utf-8"));
  const entries = cn.certifications || [];
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:      now,
    source_file:    "public/data/climate-neutral.json",
    ...
    certified_list: merged.map(r => ({
      ...
      offset_tons_year: r.offset_tons_year,
    })),
  }, null, 2));
...
main().catch(err => {
  console.error("climate-neutral-merge failed:", err);
  process.exit(1);
});
