#!/usr/bin/env node
/**
 * CCI India (Competition Commission of India) — antitrust orders, abuse of
 * dominance penalties, and merger / combination notices.
 *
 * Source: https://www.cci.gov.in/antitrust/orders
 *
 * CCI publishes order PDFs but no machine-readable feed. This fetcher reads:
 *   - JSON from --url, or
 *   - the bundled CSV fixture under scripts/fixtures/cci-india/.
 *
 * Fields per row:
 *   respondent   string
 *   action_type  string
 *   penalty_inr  number   (INR rupees, 0 for non-monetary outcomes)
 *   date         ISO date
 *   url          string
 *
 * Raw output: data/raw/cci-india/<YYYY-MM-DD>.json
 */
...
const RAW_DIR = path.join(ROOT, "data/raw/cci-india");
const FIXTURE = path.join(ROOT, "scripts/fixtures/cci-india/sample.csv");

const SOURCE_URL = "https://www.cci.gov.in/antitrust/orders";
const UA = "TruNorth-CCI-India/1.0 (+https://www.trunorthapp.com)";
...
export function rowsToRecords(rows) {
  ...
  const idx = {
    ...
    penalty_inr: header.indexOf("penalty_inr"),
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    out.push({
      ...
      penalty_inr: parseINR(r[idx.penalty_inr]),
...
export function jsonToRecords(items) {
  ...
  for (const it of items ?? []) {
    ...
    out.push({
      ...
      penalty_inr: parseINR(it.penalty_inr ?? it.fine ?? it.penalty ?? 0),
      date:        (it.date ?? it.order_date ?? it.published ?? "").slice(0, 10),
...
async function loadInput() {
  if (APPLY && URL_IN) {
    ...
    if (!res.ok) throw new Error(`CCI India fetch ${res.status} ${res.statusText}`);
...
async function main() {
  console.log(`CCI India fetcher (${APPLY ? "APPLY" : "DRY"})`);
...
if (isMain) {
  main().catch(err => { console.error("cci-india-fetch failed:", err); process.exit(1); });
}
