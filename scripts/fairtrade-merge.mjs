#!/usr/bin/env node
/**
 * Step 2 -- Merge fairtrade.json into per-company JSON.
 *
 * Reads /public/data/fairtrade.json (produced annually by
 * fairtrade-fetch.mjs) and writes the structured `fairTrade` field into
 * each matching company file under enriched.fairTrade. Honors
 * slug-aliases.json + brand-parent-map.json.
 *
 * Target schema (only set when status === "ok"):
 *   enriched.fairTrade: {
 *     isFairTradeCertified: boolean,
 *     fairTradeCategories:  string[],
 ...
 *   }
 *
 * Brands with no FTUSA match are skipped (the majority).
 *
 * Locally: node scripts/fairtrade-merge.mjs
 */
...
const FT_FILE  = path.join(ROOT, "public/data/fairtrade.json");
const COMP_DIR = path.join(ROOT, "public/data/companies");
const META_DIR = path.join(ROOT, "public/data/_meta");
const LOG_FILE = path.join(ROOT, "public/data/_meta/fairtrade-merge-log.json");
...
async function mergeOne(brandEntry, maps, now) {
  ...
  company.enriched.fairTrade = {
    isFairTradeCertified: brandEntry.is_fair_trade_certified === true,
    fairTradeCategories:  brandEntry.fair_trade_categories || [],
  ...
  company.dataLastUpdated.fairTrade = now;
  ...
  return {
    ...
    routed_via,
    status:       "merged",
    categories:   brandEntry.fair_trade_categories,
...
async function main() {
  ...
  console.log("Fair Trade merge starting...");

  const ft = JSON.parse(await fs.readFile(FT_FILE, "utf-8"));
  const entries = ft.certifications || [];
  ...
  await fs.writeFile(LOG_FILE, JSON.stringify({
    merged_at:        now,
    source_file:      "public/data/fairtrade.json",
...
main().catch(err => {
  console.error("fairtrade-merge failed:", err);
  process.exit(1);
});
